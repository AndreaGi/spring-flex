...
public class HibernateConfigProcessor extends AbstractAmfConversionServiceConfigProcessor implements BeanFactoryAware, InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        if (!this.hibernateConfigured) {
            if (BeanFactoryUtils.beanNamesForTypeIncludingAncestors(getBeanFactory(), SessionFactory.class).length > 0) {
                for (SessionFactoryImplementor sessionFactory : BeanFactoryUtils.beansOfTypeIncludingAncestors(beanFactory, SessionFactoryImplementor.class).values()) {
    ...
    protected void extractHibernateMetadata(SessionFactory sessionFactory) {
        ...
        this.classMetadata.addAll(metamodel.entityPersisters().values().stream().map(e -> e.getClassMetadata()).collect(Collectors.toSet()));
        this.collectionMetadata.addAll(metamodel.collectionPersisters().values().stream().map(c -> c.getCollectionMetadata()).collect(Collectors.toSet()));
